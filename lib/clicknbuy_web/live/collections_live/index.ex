defmodule ClicknbuyWeb.CollectionsLive.Index do
          ...
          <div class="mx-auto max-w-[1500px]">
            ...
            <%= if @results == [] do %>
              <div class="mt-10 border border-gray-100 bg-white px-6 py-20 text-center">
                <p class="font-heading-brand text-lg font-bold text-ink">
          ...
          <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-6">
            <%= for {collection, index} <- Enum.with_index(@collections, 1) do %>
              <a
                ...
                class={[
                  "group relative min-h-72 overflow-hidden rounded-2xl bg-gray-100 transition hover:shadow-xl sm:min-h-80 lg:min-h-0",
                  collection_grid_span(index, length(@collections))
                ]}
              >
                <%!-- Cover image --%>
                <%= if collection.image not in [nil, ""] do %>
                  ...
                  />
                <% else %>
                  <div class="absolute inset-0 flex h-full w-full items-center justify-center bg-gradient-to-br from-gray-200 to-gray-300">
  ...
  defp collection_grid_span(index, count) do
    remainder = rem(count, 3)

    cond do
      remainder == 1 and index == count -> "lg:col-span-6 lg:aspect-[2/1]"
      remainder == 2 and index > count - 2 -> "lg:col-span-3 lg:aspect-[4/3]"
      true -> "lg:col-span-2 lg:aspect-[4/3]"
    end
  end
end
